...
#makign the bacgkrouydn image for this
    ...
    create_question_canvas.create_text(200, 180, text=question_text, font=("Arial",14), fill="black", width=360)

    create_question_canvas.create_text(200,150 , text="Please Answer The Followign Question!", font= ("fixdays", 16), fill="white")
    ...
    if image_path:
        ...
        create_question_canvas.create_image(200, 180, image=img)  # centred
        popup.image_label = img  # keep a reference to avoid garbage collection
...
def start_game(mode,username_text):
...
#the game scorebored shows
   ...
   # game_window.after (next_pop_up, lambda: random_question_pop_up(mode, game_window, canvas,pacman))
    ...
    def save_scorew(username, score):
        with open("scoreboard_data.txt", "a")as file:
