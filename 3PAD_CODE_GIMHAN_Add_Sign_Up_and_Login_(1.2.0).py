...
def chose_mode():
    ...
    def handle_submit(mode):
        ...
        if not validate_username_info(username_text):
            messagebox.showwarning("Input Error", "Please enter your login username.")

        if not validate_login_password_info():
            messagebox.showwarning("Input Error", "Please enter your login password.")
            return
        
    #Letting he user login
    #Until the login is sucessfull it will stay as false and deny acces to play
    ...
    sign_up__button = tk.Button(window, text="Sigh up", command=lambda: sign_up_window(),  font=("fixedsys", 12), bg=("purple"), fg=("White"), width=20)
...
#Finding the cordinates of he canvas
    def get_coordinates(event):
        print(f"Mouse clicked at: {event.x}, {event.y}/n")
