...
# Create Canvas
window = tk.Canvas(window, width=500, height=450)
window.pack()

#chosign what mode the usar will use
def chose_mode():
    username_info = username_info_entry.get()

    if not validate_username_info(username_info):
    ...
    data.append(save_details)
    save_data(gamer_tag, username_info)
    messagebox.showinfo("Success", f"Game Progress saved successfully. GamerTag: {gamer_tag}")

    #Get da name
    #get the user namebg
    Label(window, text="Enter Username:").pack()
    username_info_entry = Entry(window)
    username_info_entry.pack()
    Button(window, text="Submit", command=chose_mode).pack()
    
    # Show difficulty buttons
    tk.Button(window, text="Easy", command=start_easy).pack()
    tk.Button(window, text="Medium", command=start_medium).pack()
    tk.Button(window, text="Hard", command=start_hard).pack()
...
# Add the valid hire to the list
    #save_details = {
        #"username":username_info,

    #}
   # data.append(save_details)
    #save_data(receipt_number, username_info)
    #messagebox.showinfo("Success", f"Game Progress saved sucsessfully,GamerTag: {gamer_tag}")



#adding ususal ghost speed
ghost_speed = 1

#Addiing other ghost speeds accoridng to mode
def start_easy():
    global ghost_speed 
    ghost_speed = 1
    start_game()
...
def save_data(gamer_tag, username_info):
    if gamer_tag != "" and username_info != "":
        with open("data.txt", "a") as file:
            file.write(f"{gamer_tag},{username_info}\n")
    else:
        messagebox.showwarning("Warning", "Please enter a GamerTag and Username.")
